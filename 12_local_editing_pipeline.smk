...
rule reditools_2:
	input: bam = INFOLDER + "{SAMPLE}.bam",
		cov = OUTFOLDER + "{SAMPLE}.rediout_cov"
	...
	shell:
		"module load reditools;"
                ...
                "mpirun -np {threads} {reditools}/src/cineca/parallel_reditools.py -f {input.bam} -r {params.REF} -S -s {STRAND} -ss {LOCUS_DP} -mrl 50 -q 10 -bq 20 -C -T 2 -c -m /sc/arion/projects/ad-omics/data/references/editing/homopolymeric_sites_hg38.txt -os {LOCUS_DP} -t `pwd`/new_temp_results/ -Z {params.REFINDEX} -G {input.cov} -D `pwd`/new_temp_cov/ -o {wildcards.SAMPLE}_out;"
