...
option_list <- list(make_option(c('--input'), help = '', default = ''),
                    ...
                    make_option(c('--siteDepth'), default = 5, help = 'required read coverage of the locus'))
...
sampleID <- gsub(".knownES.out", "", sampleFile)
...
#require editing site to be covered by >=5 (default) reads and >=3 (default) reads covering the edited allele
df_filt <- 
  df_long3 %>%
